...
rahe.sis.regenerate = {
	...
	getThumbnails: function () {
		...
		if (
			inputs.length !=
			jQuery( 'input.thumbnails[type="checkbox"]' ).length
		) {
	...
	getPostTypes: function () {
		...
		if (
			inputs.length !=
			jQuery( 'input.post_types[type="checkbox"]' ).length
		) {
	...
	checkStartRegenerating: function () {
		if ( jQuery( '.notSaved' ).length > 0 ) {
			var confirmation = confirm( window.sis.notSaved );
	...
	startRegenerating: function () {
		...
		jQuery.ajax( {
			...
			beforeSend: function () {
				...
				// Display the message
				self.setMessage( window.sis.reading );
			},
			success: function ( r ) {
				if ( typeof r !== 'object' ) {
					self.reInit();
					self.setMessage( window.sis.phpError );
					return false;
				}
	...
	regenItem: function () {
		...
		if ( this.total == 0 || _.isUndefined( this.total ) ) {
			this.reInit();
			this.setMessage( window.sis.noMedia );
			return false;
		}
		...
		if ( this.curr >= this.total ) {
			...
			this.setMessage(
				sis.done +
					this.curr +
					' ' +
					window.sis.messageRegenerated +
					window.sis.startedAt +
					' ' +
					this.dateScript.getHours() +
					':' +
					this.dateScript.getMinutes() +
					':' +
					this.dateScript.getSeconds() +
					window.sis.finishedAt +
					' ' +
					now.getHours() +
					':' +
					now.getMinutes() +
		...
		jQuery.ajax( {
			...
			success: function ( r ) {
				...
				if ( ! r.src || ! r.time || r.error || typeof r !== 'object' ) {
					...
					self.timeScript.push(
						parseFloat( r.time.replace( ',', '.' ), 10 )
					);
					...
					time = self.s2t(
						Math.abs( dateEnd.getTime() - curDate.getTime() ) / 1000
					);
	...
	s2t: function ( secs ) {
		...
		if ( secs > 86399 ) {
			s =
				Math.floor( ( t - Date.parse( '1/1/70' ) ) / 3600000 ) +
				s.substr( 2 );
		}
		return s;
	},
	reInit: function () {
		...
		this.buttonRegenerate.prop( 'disabled', false );
		this.thumb.hide();
		this.progress.hide();
...
rahe.sis.sizes = {
	...
	register: function ( e, el ) {
		...
		row = row( {
			size: window.sis.size,
			size_name: name,
			maximumWidth: window.sis.maximumWidth,
			maximumHeight: window.sis.maximumHeight,
			customName: window.sis.customName,
			crop: window.sis.crop,
			crop_positions: window.sis.crop_positions,
			show: window.sis.show,
			deleteImage: window.sis.deleteImage,
			validateButton: window.sis.validateButton,
		} );
	...
	deleteSize: function ( e, el ) {
		...
		var confirmation = confirm( window.sis.confirmDelete );
		...
		if ( ! parentTable.hasClass( 'ajaxing' ) ) {
			jQuery.ajax( {
				...
				success: function ( result ) {
					...
					parent
						.find( '.add_size' )
						.removeClass( 'validate_size' )
						.hide()
						.children( '.ui-button-text' )
						.text( window.sis.update );

					clearTimeout( timer );
					// Remove classes after 3 seconds
					timer = setTimeout( function () {
						parent.removeClass(
							'errorAdding notChangedAdding successAdding'
						);
					}, 2 * 1000 );
				},
			} );
	...
	ajaxUnregister: function ( el ) {
		...
		var self = this,
			n = jQuery( el )
				.closest( 'tr' )
				.find( 'input[name="image_name"]' )
				.val(),
			wp_nonce = jQuery( el )
				.closest( 'tr' )
				.find( 'input.deleteSize' )
				.val();
	...
	addToArray: function ( n, w, h, c, s ) {
		...
		if ( testRow.length != 0 ) {
			...
			newRow = jQuery(
				'#sis-regen .wrapper > table#sis_sizes > tbody > tr:first'
			)
				.clone()
				.attr( 'id', 'sis-' + n );
		}

		c = ! _.isUndefined( window.sis.crop_positions[ c ] )
			? window.sis.crop_positions[ c ]
			: window.sis.fl;
		...
		newRow
			...
			.attr( 'for', n )
			.end()
			.find( 'input.thumbnails' )
			.val( n )
			.attr( 'id', n )
			.end()
			.find( 'th:nth-child(2) > label' )
			.text( n )
			.end()
			.find( 'th:nth-child(3) > label' )
			.text( w + 'px' )
			.end()
			.find( 'th:nth-child(4) > label' )
			.text( h + 'px' )
			.end()
			.find( 'th:nth-child(5) > label' )
			.text( c );
		...
		newRow
			.removeClass( 'errorAdding notChangedAdding successAdding' )
			.addClass( s );
...
rahe.sis.template = _.memoize( function ( id ) {
	...
	return function ( data ) {
		compiled =
			compiled ||
			_.template( jQuery( '#sis-' + id ).html(), null, options );
...
jQuery( function () {
	...
	jQuery( '#ajax_thumbnail_rebuild' ).on( 'click', function () {
