...
this.dateScript=new Date;jQuery.ajax({url:sis.ajaxUrl,type:"POST",dataType:"json",data:"action=sis_ajax_thumbnail_rebuild&do=getlist"+b.post_types+"&nonce="+a,beforeSend:function(){jQuery("#ajax_thumbnail_rebuild").attr("disabled",!0);b.setMessage(sis.reading);b.getThumbnails();b.getPostTypes()},success:function(a){if("object"!==typeof a)return b.reInit(),b.setMessage(sis.phpError),!1;jQuery("#time").show();b.list=a;b.curr=0;jQuery(".progress").show();b.regenItem()}})},regenItem:function(){var b=
this,a=jQuery("input.regen").val();if(!this.list)return this.reInit(),this.setMessage(sis.noMedia),!1;this.curr>=this.list.length?(a=new Date,this.reInit(),this.setMessage(sis.done+this.curr+" "+sis.messageRegenerated+sis.startedAt+" "+a.getHours()+":"+a.getMinutes()+":"+a.getSeconds()+sis.finishedAt+" "+a.getHours()+":"+a.getMinutes()+":"+a.getSeconds())):(this.setMessage(sis.regenerating+(this.curr+1)+sis.of+this.list.length+" ("+this.list[this.curr].title+")..."),jQuery.ajax({url:sis.ajaxUrl,type:"POST",
dataType:"json",data:"action=sis_ajax_thumbnail_rebuild&do=regen&id="+this.list[this.curr].id+this.thumbnails+"&nonce="+a,beforeSend:function(){b.percent=100*(b.curr/b.list.length);jQuery(".progress").progressbar("value",b.percent);jQuery(".progress-percent span.text").html(Math.round(b.percent)+"%").closest(".progress-percent").animate({left:Math.round(b.percent)-2.5+"%"},500)},success:function(a){if(!a.src||!a.time||a.error||"object"!==typeof a){var e="",e="object"!==typeof a?sis.phpError:a.error;
jQuery("#error_messages").addClass("error message");jQuery("#error_messages ul.messages").prepend("<li>"+e+"</li>")}else{a.message&&jQuery("#time ul.messages").prepend("<li>"+a.message+"</li>");var e=new Date,f=new Date,g=0;sum=0;jQuery("#thumb").show();jQuery("#thumb-img").attr("src",a.src);b.timeScript.push(a.time);g=b.timeScript.length;for(a=0;a<g;a++)sum+=b.timeScript[a];a=Math.round(1E3*sum/g*b.list.length);e.setTime(b.dateScript.getTime()+a);f=b.s2t((e.getTime()-f.getTime())/1E3);b.setTimeMessage(e.getHours()+
":"+e.getMinutes()+":"+e.getSeconds()+sis.or+f+sis.beforeEnd)}b.curr++;b.regenItem()}}))},s2t:function(b){b%=86400;var a=new Date(1970,0,1),c=0;a.setSeconds(b);c=a.toTimeString().substr(0,8);86399<b&&(c=Math.floor((a-Date.parse("1/1/70"))/36E5)+c.substr(2));return c},reInit:function(){jQuery("#ajax_thumbnail_rebuild").removeAttr("disabled");jQuery(".progress, #thumb").hide()}};
